...
mod tests {
    ...
    use crate::{Adb, AdbClient, Client, Device, intent, PackageManager};
    use crate::dump_util::SimplePackageReader;
    use crate::scanner::Scanner;
    use crate::types::{
        DumpsysPriority, FFPlayOptions, InputSource, InstallLocationOption, InstallOptions, KeyCode, KeyEventType, LogcatLevel, LogcatOptions, LogcatTag, MotionEvent, PackageFlags, ScreenRecordOptions,
        SELinuxType, SettingsType, UninstallOptions,
    };
    ...
    fn test_edit_file() {
        ...
        let text_editor = which::which("code");
        if let Err(err) = text_editor {
            error!("vscode not found: {err}");
            return;
        }

        let tmp = PathBuf::from("/Users/alessandro/Desktop/tvlib-aot-client.properties");
        if let Ok(mtime) = tmp.metadata().and_then(|m| m.modified()) {
            //let tmp = temp_dir().join("temp.file.txt");
            let tmp_str = tmp.to_str().unwrap();
            //let result = client.pull("/timeshift/conf/aot-client.properties", tmp_str).unwrap();

            let mut command = Cmd::builder(text_editor.unwrap())
                .args(&[
                    "-w", "--", tmp_str,
                ])
                .with_debug(true)
            ...
            let handle = std::thread::spawn(move || {
                loop {
                    match child.try_wait() {
                        Ok(Some(output)) => {
                            if output.success() {
                                if let Ok(mtime2) = tmp.metadata().and_then(|m| m.modified()) {
                                    if mtime2 > mtime {
                                        let _ = tx_cloned.send(Ok(true));
                                    } else {
                                        let _ = tx_cloned.send(Ok(false));
                                    }
                                }
                            }
                            drop(tx_cloned);
            ...
            for x in rx {
                if let Ok(true) = x {
                    info!("File updated");
                } else if let Ok(false) = x {
                    warn!("File not updated");
