...
mod tests {
	...
	async fn test_screen_record() {
		...
		match client.pull(remote_file, local_file.as_path()).await {
			Ok(t) => {
				debug!("Pull Ok: {:?}", t)
			}
			Err(e) => {
	...
	async fn test_send_events() {
		...
		shell.send_event("/dev/input/event3", 0x0001, 0x006a, 0x00000001).await.unwrap();
		shell.send_event("/dev/input/event3", 0x0000, 0x0000, 0x00000000).await.unwrap();

		// KEYCODE_DPAD_RIGHT (action UP)
		shell.send_event("/dev/input/event3", 0x0001, 0x006a, 0x00000000).await.unwrap();
		shell.send_event("/dev/input/event3", 0x0000, 0x0000, 0x00000000).await.unwrap();
	}
